...
template <typename T> static void sortOwningVec(OwningVec<T> &List) {
  if (List.empty() || std::next(List.begin()) == List.end())
    return;
  std::vector<T *> Sorted;
  for (auto &Item : List)
    Sorted.push_back(&Item);
  llvm::sort(Sorted, [](const T *A, const T *B) { return *A < *B; });
  List.clear();
...
void ScopeChildren::sort() {
  sortOwningVec(Namespaces);
  sortOwningVec(Records);
  sortOwningVec(Functions);
  sortOwningVec(Enums);
  sortOwningVec(Typedefs);
  sortOwningVec(Concepts);
  sortOwningVec(Variables);
}
} // namespace doc
} // namespace clang
